  ...
  for (let decl of varDecls) {
    targetsAndReplacements.push({
      ...
      replacementFunc: function(declNode, s, wasChanged) {
        ...
        let nodes = [];
        for (let subDeclNode of declNode.declarations) {
          var extractedId = {type: "Identifier", name: "__temp"},
              extractedInit = {
                type: "VariableDeclaration", kind: "var",
                declarations: [{type: "VariableDeclarator", id: extractedId, init: subDeclNode.init}]
              }
          nodes.push(extractedInit);

          for (let {key: keyPath} of helpers.objPropertiesAsList(subDeclNode.id, [], false)) {
            nodes.push(varDecl(
